...
# DECISION TREE RECIPE CLASSIFIER 
from sklearn.tree import DecisionTreeClassifier

# Recipe Classifier Using Decision Tree Test #########
from tree_test import test_tree_accuracy

results = test_tree_accuracy(
      transformed_X_train, transformed_X_test, y_train, y_test
)

#Recipe Classifier ################
#swept for best max_depth earlier and found 20 to be optimal
dt = DecisionTreeClassifier(max_depth=20, class_weight='balanced', random_state=67)
dt.fit(transformed_X_train, y_train)

dt_preds = dt.predict(transformed_X_test) #predictions based on the test set
accuracy = accuracy_score(y_test, dt_preds) #accuracy of comparing predictions to the actual labels
print(f"\nDecision Tree accuracy: {accuracy:.2%}")

cm = confusion_matrix(y_test, dt_preds) #confusion matrix for more insight
print("\nConfusion matrix:")
print(cm)
print("\nAll scores:") #classification report for more insight
print(classification_report(y_test, dt_preds, target_names=["bad-recipe", "good-recipe"]))
